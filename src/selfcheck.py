...
def _check_ready_ideas() -> tuple[list[str], list[str]]:
    ...
    sb_listing = PROJECT_ROOT / "assets" / "ready_ideas" / "soldier_boy_preview.png"
    if not sb_listing.is_file():
        errors.append("Soldier boy listing preview missing: assets/ready_ideas/soldier_boy_preview.png")
    checks.append("Soldier boy ready-idea listing preview file exists.")
    ...
    povj_listing = PROJECT_ROOT / "assets" / "ready_ideas" / "povjazannyj_preview.png"
    if not povj_listing.is_file():
        errors.append("Povjazannyj listing preview missing: assets/ready_ideas/povjazannyj_preview.png")
    checks.append("Povjazannyj ready-idea listing preview file exists.")
    ...
    add_photo_listing = PROJECT_ROOT / "assets" / "ready_ideas" / "add_photo_object_in_scene_preview.png"
    if not add_photo_listing.is_file():
        errors.append(
            "Add photo / object-in-scene listing preview missing: "
            "assets/ready_ideas/add_photo_object_in_scene_preview.png"
        )
    checks.append("Add photo (object in scene) ready-idea listing preview file exists.")
