...
async def cb_group_help(callback: CallbackQuery) -> None:
    ...
    text = (
        ...
        "/report\n"
        "— Сводка поддержки за 7 дней: сколько обращений и закрытий, сколько тикетов открыто сейчас, "
        "разбивка по меткам (/tag), оценки клиентов и распределение по звёздам. "
        "Это не статистика пользователей основного бота — её смотрите в основном боте: /stats."
    )
    if callback.message:
        await callback.message.answer(text)
    await callback.answer()
