...
READY_IDEA_ITEMS: dict[str, list[tuple[str, str, str, int]]] = {
    ...
    "games": [
        ...
        (
            ...
            "NEGATIVE: cartoon, chibi, anime, Pixar/DreamWorks 3D, Fortnite/disney-stylized proportions, cel-shaded or toon-shaded look, WoW classic in-game shader / character-select doll, exaggerated mobile-RPG splash art, oversized weapons as toys, plastic skin, floating head only, extra people, readable copyrighted logos or game UI, HUD/health bars, watermark text, flat clipart, duplicate faces, generic racial face that ignores the reference likeness.",
            1,
        ),
    ],
    "colors": [
...
def _ready_idea_listing_photo_path(title: str) -> Path | None:
    ...
    Только для листания этой идеи; при выходе в категории/загрузку фото подставляется start_banner
    через _edit_ready_nav_message + _ready_categories_listing_photo().
    """
    t = title.strip()
